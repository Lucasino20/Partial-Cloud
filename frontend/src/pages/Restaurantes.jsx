  ...
  return (
    <div className="container animate-fade-in">
      <div style={{ textAlign: 'center', marginBottom: '50px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem' }}>Selecciona un plato para realizar un pedido real.</p>
      </div>
      
      {msg && <div style={{ background: 'rgba(34,197,94,0.1)', color: '#4ade80', padding: '16px', borderRadius: '12px', marginBottom: '24px', border: '1px solid rgba(34,197,94,0.3)', textAlign: 'center', fontWeight: 'bold' }}>{msg}</div>}

      <div className="grid-responsive" style={{ gap: '30px' }}>
        {rests.map(r => (
          <div className="card hover-card" key={r.id || r._id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '24px', overflow: 'hidden' }}>
            <div style={{ position: 'relative' }}>
              <img src={r.img || "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=600&h=300&fit=crop"} className="card-img" alt="rest" style={{ height: '200px', objectFit: 'cover' }} />
            ...
            <div className="card-content" style={{ padding: '25px' }}>
              <h2 style={{ margin: '0 0 20px 0', fontSize: '1.4rem' }}>{r.nombre}</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {r.platos && r.platos.slice(0,4).map(p => (
                   <div key={p.id || p._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 15px', background: 'rgba(0,0,0,0.3)', borderRadius: '12px', transition: 'all 0.2s ease' }}>
                     <span style={{ fontWeight: '500' }}>{p.nombre}</span>
                     <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                       <span style={{ color: 'var(--primary-color)', fontWeight: 'bold' }}>S/{Number(p.precio).toFixed(2)}</span>
                       <button className="btn" style={{ padding: '6px 16px', fontSize: '0.85rem', borderRadius: '20px' }} onClick={()=>{setOrderingDish(p); setOrderRest(r);}}>Pedir</button>
                     </div>
                   </div>
                ))}
              </div>
